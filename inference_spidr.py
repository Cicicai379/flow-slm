...
from trainer_spidr import LanguageModeling
...
def parse_args():
    parser = argparse.ArgumentParser(description="GSLM SPIDR inference")
...
def prepare_sampler_and_processor(lm, conf, args, device="cuda"):
    frame_rate = 12.5
    # silence_indices are Mimi-codebook-specific; SPIDR tokens have a different vocab (size 268), so clear them
    sampler = Sampler(lm.gslm_pipeline, lm.loss_fn, frame_rate=frame_rate, silence_indices=[]).to(device)
...
def run_unconditional(args, sampler, processor):
    # SPIDR codebook_size=256; eos token = 256 + n_special_tokens - 1
    codec_size = 256
